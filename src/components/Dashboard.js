...
function Dashboard({ user, userType }) {
  ...
  const carregarDadosProfessor = useCallback(async () => {
    ...
    const [turmasSnapshot, alunosSnapshot, presencasSnapshot] = await Promise.all([
      getDocs(query(collection(db, 'turmas'), where('professorId', '==', professorId))),
      getDocs(query(collection(db, 'alunos'), where('professorId', '==', professorId))),
      getDocs(collection(db, 'presencas')),
    ]);
    ...
    const idsAlunos = new Set(alunosSnapshot.docs.map((item) => item.id));
    setPresencas(
  ...
  const renderAluno = () => {
    ...
    const presentes = presencas.filter((item) => item.status === 'presente').length;
    const atrasados = presencas.filter((item) => item.status === 'atrasado').length;
        ...
        {!aluno ? (
          ...
          <>
            ...
            <div className="stats-grid student-stats">
              ...
              <div className="stat-card"><span>Atrasos</span><strong>{atrasados}</strong></div>
  ...
  const renderConteudo = () => {
    ...
    switch (pagina) {
      ...
      case 'turmas':
        return <Turmas turmas={turmas} reload={carregarDados} />;
      case 'alunos':
